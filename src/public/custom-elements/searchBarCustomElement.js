        ...
        LiteAPI.SearchBar.create({
          ...
          onSearchClick: (searchData) => {
            ...
            const occupancies = JSON.parse(
              atob(String(searchData?.occupancies ?? "").trim())
            );
            ...
            window.top.location.assign(
              new URL(
                `hotels?${new URLSearchParams({
                  ...
                  name: String(
                    searchData?.place?.description ?? searchData?.query ?? ""
                  ).trim(),
                  checkin: dateText(
                    searchData?.checkin || searchData?.dates?.start
                  ),
                  checkout: dateText(
                    searchData?.checkout || searchData?.dates?.end
                  ),
                  rooms: String(rooms.length || 1),
                  adults: rooms
                    .map((r) => String(number(r?.adults, 1)))
                    .join(","),
                  children: rooms
                    .flatMap((r, i) =>
                      (r?.children ?? []).map(
                        (age) => `${i + 1}_${number(age, 0)}`
                      )
                    )
                    .join(",")
                })}`,
